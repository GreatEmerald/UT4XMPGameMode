...
AXTeamInfo::AXTeamInfo(const FObjectInitializer& ObjectInitializer)
: Super(ObjectInitializer)
{
	TeamEnergyInitial = 30;
	TeamEnergy = TeamEnergyInitial;
	TeamEnergyMax = 120;
}

void AXTeamInfo::GetLifetimeReplicatedProps(TArray< FLifetimeProperty > & OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);

	// Replicate to everyone
	DOREPLIFETIME(AXTeamInfo, TeamEnergy);
}
