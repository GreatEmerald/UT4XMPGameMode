...
void AXPlayerState::GetLifetimeReplicatedProps(TArray< FLifetimeProperty > & OutLifetimeProps) const
{
	...
	DOREPLIFETIME(AXPlayerState, PlayerLevel);
}

void AXPlayerState::ServerRequestGoSpectate_Implementation()
{
	AXGameMode* Game = GetWorld()->GetAuthGameMode<AXGameMode>();
	AXPlayerController* Controller = Cast<AXPlayerController>(GetOwner());
	Game->SendToSpectate(Controller);
}

bool AXPlayerState::ServerRequestGoSpectate_Validate()
{
	return true;
}
